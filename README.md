...
- **나의 사물함 정보 페이지(`/my-locker.html`)**
  - 학번 / 이름 / 사물함 번호 확인
  - 물품 리스트 관리 (추가/수정/삭제, **드래그 정렬 가능**)
  - 메모 저장
  - **사물함 비우기(사용 종료)**
    - 학번 + 확인코드 재입력으로 본인 확인 후 처리
    - 비우기 완료 시 신청 정보 삭제 및 확인코드 폐기
...
- 확인코드는 **서버에 BCrypt 해시 형태로 저장** (원문 저장 X)
- 조회/비우기 요청 시 확인코드를 서버에서 BCrypt로 검증
...
### ✅ 현재 운영(배포) 상태
- VPS(서버 대여) 환경에 배포하여 서비스 운영 중
- Docker 기반으로 컨테이너 실행 및 재배포/업데이트 진행

### 🔧 향후 개선 계획
- HTTPS 적용 및 도메인 연결(인증서 자동 갱신 포함)
- 관리자 로그/감사 기록(승인·거절·초기화·비밀번호 변경 이력)
- 확인코드 재발급 기능 + 재발급 제한(남용 방지)
- 모바일 UI 최적화(사물함 정보 카드/리스트/모달 반응형 개선)
- 배포 안정화
  - 무중단/자동 재시작(예: compose + restart policy)
  - 환경변수(.env) 기반 설정 분리 및 시크릿 관리
  - 백업/복구 전략(DB dump 주기, 장애 대응 절차)
